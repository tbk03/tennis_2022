...
top_twenty_countries %>%
  ...
  mutate(n_bin = cut(n, breaks = c(0, 10, 20, 50, 100, Inf))) %>% 
  ggplot() +
  geom_tile(aes(x = year, 
                y = factor(country, levels = rev(levels)),
                fill = n_bin),
  ...
  # palette: https://gka.github.io/palettes/#/5|s|c5c8e2,000da8|ffffe0,ff005e,93003a|1|1
  scale_fill_manual(values = c('#c5c8e2', '#a499d5', '#7f6bc6', '#553eb7', '#000da8')) +
...
check_top_50 <- function(year, country){
  ...
  selector <- top_50_countries_by_year$year == 2010 & 
